...
class Model(QtCore.QObject):
    ...
    conn = sqlite3.connect("/home/ubuntu/yolov5/log.db", isolation_level=None)
    curs = conn.cursor()
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    for d in det:
                        c = int(d[5].item())
                        if names[c] == 'sliding':  # 슬라이싱 인식 시

                            justnow = datetime.datetime.now()
                            self.fallTimeList.append(
                                justnow)  # 리스트에 시간을 넣음

                            if len(self.fallTimeList) >= 2:  # 2번 이상 인식한 경우
                                # 두 사건사이의 시간을 구함
                                time = self.fallTimeList[-1] - \
                                    self.fallTimeList[0]
                                if int(time.total_seconds()) >= 10:  # 두 사건 사이의 시간이 10초 이상이면 초기화
                                    self.fallTimeList.clear()

                            # 두 사건 사이의 시간이 1초면 리스트 초기화 후 log저장
                                if int(time.total_seconds()) == 1:
                                    self.fallTimeList.clear()
                                    self.now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')

                                    self.curs.execute(
                                        "INSERT INTO log VALUES(?,'Sliding')", (self.now,))

                                    self.conn.commit()
                                    db_title.append(
                                        str(self.now)+"  fall occurrence!!")

                                else:
                                    datetime.timedelta(0, 0, 0, 0, 0, 0, 0)
                ...
                dst = cv2.resize(im0, dsize=(1280, 720),
...
if __name__ == "__main__":
    ...
    model = Model()
    model.VideoSignal.connect(image_viewer.setImage)
    title = QtWidgets.QLabel('FALL DETECTION SYSTEM')
    title.setStyleSheet("color:black;font-weight:bold;")
    title.setFont(QFont("Roboto", 17))
    title.setAlignment(QtCore.Qt.AlignCenter)
    db_title = QtWidgets.QTextEdit('')
    taking_over = QtWidgets.QTextEdit('')
    ...
    horizontal_layout.addWidget(db_title)

    vertical_layout.addLayout(horizontal_layout)

    vertical_layout.addWidget(taking_over)
